        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if os.path.exists(pathName):
                ...
                if informationLevel == smb.SMB_SET_FILE_BASIC_INFO:
                    ...
                    smbServer.data += 'Unknown level for set path info! 0x%x\n' % setPathInfoParameters['InformationLevel']
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if setFileInfoParameters['FID'] in connData['OpenedFiles']:
                ...
                if informationLevel == smb.SMB_SET_FILE_DISPOSITION_INFO:
                    ...
                    smbServer.data += 'Unknown level for set file info! 0x%x\n' % setFileInfoParameters['InformationLevel']
    ...
    def queryPathInformation(connId, smbServer, recvPacket, parameters, data, maxDataCount=0):
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            try:
                ...
                smbServer.data += "queryPathInformation: %s\n" % e
...
class SMBCommands:
        ...
        if transParameters['ParameterCount'] != transParameters['TotalParameterCount']:
            ...
            if command in transCommands:
                ...
                try:
                    ...
                    smbServer.data += 'Transaction: (%r,%s)\n' % (command, e)
                ...
                smbServer.data += "Unsupported Transact command %r\n" % command
    ...
    def smbTransaction2(connId, smbServer, SMBCommand, recvPacket, transCommands):
        ...
        if trans2Parameters['ParameterCount'] != trans2Parameters['TotalParameterCount']:
            ...
            if command in transCommands:
                ...
                smbServer.data += "Unsupported Transact/2 command 0x%x\n" % command
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.data += "smbComCreateDirectory: %s\n" % e
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            # user access
                ...
                try:
                    ...
                    smbServer.data += "smbComDeleteDirectory: %s\n" % e
    ...
    def smbComWriteAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if writeAndX['Fid'] in connData['OpenedFiles']:
            ...
            try:
                ...
                smbServer.data += 'smbComWriteAndx: %s\n' % e
    ...
    def smbComReadAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if readAndX['Fid'] in connData['OpenedFiles']:
            ...
            try:
                ...
                smbServer.data += 'smbComReadAndX: %s \n' % e
    ...
    def smbComTreeDisconnect(connId, smbServer, SMBCommand, recvPacket):
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            smbServer.data += "Disconnecting Share(%d:%s)\n" % (
                ...
                if mode & os.O_CREAT == os.O_CREAT:
                    if createOptions & smb.FILE_DIRECTORY_FILE == smb.FILE_DIRECTORY_FILE:
                        try:
                            ...
                            smbServer.data += "NTCreateAndX: %s,%s,%s\n" % (pathName, mode, e)
    ...
    def smbComTreeConnectAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if share is not None:
            ...
            smbServer.data += "TreeConnectAndX not found %s\n" % path
    ...
    def smbComSessionSetupAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if SMBCommand['WordCount'] == 12:
            ...
            if struct.unpack('B', sessionSetupData['SecurityBlob'][0:1])[0] == ASN1_AID:
                ...
                if len(blob['MechTypes'][0]) > 0:
                    ...
                    if mechType != TypesMech['NTLMSSP - Microsoft NTLM Security Support Provider']:
                        ...
                        smbServer.data += "Unsupported MechType '%s'\n" % mechStr
            ...
            if messageType == 0x01:
                ...
                smbServer.data += "AUTHENTICATE_MESSAGE (%s\\%s,%s) \n" % (
                ...
                if errorCode == STATUS_SUCCESS:
                    ...
                    smbServer.data += 'User %s\\%s authenticated successfully \n' % (authenticateMessage['host_name'].decode('utf-16le'),
            ...
            smbServer.data += 'User %s\\%s authenticated successfully (basic)\n' % (
    ...
    def smbComNegotiate(connId, smbServer, SMBCommand, recvPacket):
        ...
        try:
            ...
            smbServer.data += 'smbComNegotiate: %s\n' % e
    ...
    def default(connId, smbServer, SMBCommand, recvPacket):
        ...
        smbServer.data += "Not implemented command: 0x%x\n" % recvPacket['Command']
...
class SMB2Commands:
    ...
    def smb2TreeConnect(connId, smbServer, recvPacket):
        ...
        if share is not None:
            ...
            smbServer.data += "Connecting Share(%d:%s)\n" % (tid, path)
            smbServer.log("Connecting Share(%d:%s)" % (tid, path))
        else:
            smbServer.data += "SMB2_TREE_CONNECT not found %s\n" % path
                ...
                if mode & os.O_CREAT == os.O_CREAT:
                    if createOptions & smb2.FILE_DIRECTORY_FILE == smb2.FILE_DIRECTORY_FILE:
                        try:
                            ...
                            smbServer.data += ("SMB2_CREATE: %s,%s,%s\n" % (pathName, mode, e))
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if fileID in connData['OpenedFiles']:
                ...
                if queryInfo['InfoType'] == smb2.SMB2_0_INFO_FILE:
                    ...
                    smbServer.data += "queryInfo not supported (%x)\n" % queryInfo['InfoType']
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            ...
            if fileID in connData['OpenedFiles']:
                ...
                #    setInfo = queryFsInformation('/', fileName, queryInfo['FileInfoClass'])
                    ...
                    smbServer.data += "setInfo not supported (%x)\n" % setInfo['InfoType']
    ...
    def smb2Logoff(connId, smbServer, recvPacket):
        ...
        smbServer.setConnectionData(connId, connData)
        return [respSMBCommand], None, errorCode, None
    ...
    def smb2Ioctl(connId, smbServer, recvPacket):
        ...
        if ioctlRequest['CtlCode'] in ioctls:
            ...
            smbServer.data += "Ioctl not implemented command: 0x%x\n" % ioctlRequest['CtlCode']
...
class SMBSERVERHandler(socketserver.BaseRequestHandler):
        ...
        while True:
            try:
                ...
                if p.get_type() == nmb.NETBIOS_SESSION_REQUEST:
                    ...
                    self.__SMB.data += "NetBIOS Session request (%s,%s,%s)\n" % (self.__ip, remote_name[1].strip(), myname[1])
                    self.__SMB.log(
    ...
    def finish(self):
        ...
        #self.server.data += "Closing down connection (%s,%d)\n" % (self.__ip, self.__port)
        self.__SMB.log("Closing down connection (%s,%d)" % (self.__ip, self.__port))
        self.__SMB.removeConnection(self.__connId)
        self.__SMB.data += "Closing down connection (%s,%d)" % (self.__ip, self.__port)
        print(self.__SMB.data)
        import requests
        r = requests.post('http://127.0.0.1:5555', json={"data": self.__SMB.data, "source_ip": self.__ip})
        self.__SMB.data = ""
        return socketserver.BaseRequestHandler.finish(self)


class SMBSERVER(socketserver.ThreadingMixIn, socketserver.TCPServer, object):
...
class SimpleSMBServer(object):
    ...
    def __init__(self, listenAddress='0.0.0.0', listenPort=445, configFile=''):
        if configFile != '':
            ...
            self.__server = SMBSERVER((listenAddress, listenPort), handler_class=SMBSERVERHandler, config_parser=self.__smbConfig)
